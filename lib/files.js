 ...
 */
exports.save = function saveArticle(article, folder, isCategory) {

  var content = article.md;
  var title = sanitize(article.title, { replacement: '-' });

  var destination;
  var dbFolder = (folder) ? path.resolve(folder) : path.resolve('content');

  folder = path.join(dbFolder, '_content');
  folder = (isCategory) ? path.join(folder, 'categories') : folder;
