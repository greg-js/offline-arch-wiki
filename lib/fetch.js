...
function buildArticleObjectList(opts, doWikitext) {
  return new Promise(function makePromise(resolve, reject) {
    ...
    if (doWikitext) { delete options.rvparse; }
    ...
    return fetchBatch(options, [], function getPages(err, objectList) {
      ...
      articleObjectList = objectList.map(function mapObjects(article) {
        ...
        if (doWikitext) {
          obj.description = makeDescription(toMD(article.revisions[0].content));
        }
 ...
 **/
function buildSyncList(opts, date, doWikitext) {
  ...
  return new Promise(function makePromise(resolve, reject) {
    var options = {
      ...
      grclimit: opts.grclimit || (doWikitext) ? '50' : '1',
    ...
    return fetchBatch(options, [], function getPages(err, objectList) {
      ...
      articleObjectList = objectList.map(function mapObjects(article) {
        ...
        if (doWikitext) {
          obj.description = makeDescription(article.revisions[0].content);
        }
