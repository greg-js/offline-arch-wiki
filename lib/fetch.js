...
function buildArticleObjectList(urlList, opts) {
  ...
  var options = {
    ...
    prop: opts.prop || 'text|categories',
    format: opts.format || 'json',
    formatversion: opts.formatversion || '2',
  };
  ...
  var articlePromises = urlList.map(function makePromises(article) {
    return new Promise(function makePromise(resolve, reject) {
      var articleObject = {
        ...
        url: article.fullurl,
        categories: article.categories.filter(function filterCats(e) { return !e.hasOwnProperty('hidden'); }).map(function mapCats(e) { return e.category; }).join('|'),
      };
