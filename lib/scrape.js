 ...
 **/
exports.article = function scrapeArticle(articleObj, isCategory) {
  var request = require('request-promise');
  var url = articleObj.url;

  return request(url).then(function successScrape(htmlString) {
    var $ = cheerio.load(htmlString);
    var firstP = $('#mw-content-text').children('p').first().text();
    // if first paragraph starts with "From", make the description the second paragraph instead of the first
    var description = (/from/i.test(firstP.split(' ')[0])) ? $('#mw-content-text').children('p').first().next().text() : firstP;

    return {
      title: sanitize($('#firstHeading').text(), { replacement: '-' }),
