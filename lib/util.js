...
/**
 * Cleans a description, stripping it from wikitext/md markup
 **/
...
/**
 * Sanitizes filename and translates language name into English
 * @param {String} filename
 * @returns {String} sanitized
 **/
function sanitize(filename) {
  var lang = detectLanguage(filename);
  var translated = (lang === 'english') ? filename : filename.replace(languages[lang], lang);

  return sanitizeFilename(translated, { replacement: '-' });
}
