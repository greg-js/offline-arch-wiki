...
module.exports = function htmlToMD(html) {
  ...
  var boringDivConverter = {
    filter: function findNodes(node) {
      return node.tagName === 'DIV' && (/siteSub|jump\-to\-nav|catlinks/i.test(node.id) || /noprint/i.test(node.className));
    },
  ...
  return toMarkdown(html, { gfm: true, converters: [spanConverter, boringDivConverter, divConverter, preConverter, ddConverter, dlDtConverter] });
};
